...
这个repo用于实现上述算法/数据结构的python语法标准将是python2.7（第十章前）/3.6（第十章后），只提供源代码。

如果算法导论（第三版）涉及了一些C++ SL/STL 或者 其他语言的标准库 已有基础结构，并且书中更关注这些数据结构的实现细节，那么这里将不使用标准库内容实现。

对于另一些标准库没有的算法和数据结构，这里具体实现它们时可能会使用C++（或其他语言）标准库的一些基础的内容（比如线性表）。
...
除了上述语言，还可能有来自github社区贡献的其他语言的实现（比如python）。
